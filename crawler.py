...
seed = seed_input
visited_channels = seed_input
collected_channels = seed_input
iteration_channels = seed_input # The output channels in iteration i will be the input channels of iteration i+1
...
for i in range(NUM_ITERATIONS):
    ...
    print(seed)
    visited_channels += found_channels # Error: seed is also extended
    print(seed)
    # Rank the groups based on a QUERY
    print('Ranking channels..')
    ranked_channels, avg_score = model.rank(found_channels, QUERY) # Error: seed is also extended
    print(seed)
...
# with open('collected_channels_test.csv', 'w') as f:
#     w = csv.writer(f)
#     w.writerow(collected_channels)
# with open('seed_test.csv', 'w') as f:
#     w = csv.writer(f)
#     w.writerow(seed)
print(len(seed))
